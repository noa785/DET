...
export default function ReportsPage() {
  ...
  return (
    <div className="p-6 max-w-[1400px] mx-auto">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-[var(--text)] mb-1">📋 Reports Center</h1>
        <p className="text-[var(--text-2)] text-sm">Generate and download detailed reports</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-8">
        {REPORT_TYPES.map((rt) => (
          ...
          >
            ...
            <div className="font-semibold text-sm text-[var(--text)]">{rt.label}</div>
            <div className="text-xs text-[var(--text-2)] mt-1 font-medium">{rt.labelAr}</div>
          </button>
        ))}
      </div>
      ...
      <div className="bg-slate-800 rounded-xl border border-slate-700 shadow-sm mb-6">
        <div className="px-6 py-4 rounded-t-xl border-b border-slate-700 bg-slate-700/50">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-[var(--text-2)] mt-0.5">{currentReport.description}</p>
            </div>
            <button
              onClick={handleDownload}
              disabled={downloading}
              className="
                flex items-center gap-2 px-5 py-2.5 bg-green-600 text-[var(--text)] rounded-lg
        ...
        <div className="px-6 py-4 flex flex-wrap gap-3 border-b border-slate-700 bg-slate-800/50">
          ...
          <select
            ...
            className="px-3 py-2 border border-slate-600 rounded-lg text-sm bg-slate-700 text-[var(--text)] focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500 outline-none"
          >
          ...
          <input
            ...
            className="px-3 py-2 border border-slate-600 rounded-lg text-sm bg-slate-700 text-[var(--text)] focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500 outline-none [color-scheme:dark]"
            ...
            <table className="w-full text-sm">
              <thead className="bg-slate-700/50 sticky top-0">
                <tr>
                  {selectedReport === "governance" ? (
                    <>
                      <th className="px-4 py-3 text-left font-semibold text-[var(--text-2)]">Title</th>
                      <th className="px-4 py-3 text-left font-semibold text-[var(--text-2)]">Unit</th>
                      <th className="px-4 py-3 text-left font-semibold text-[var(--text-2)]">Type</th>
                      <th className="px-4 py-3 text-left font-semibold text-[var(--text-2)]">Status</th>
                      <th className="px-4 py-3 text-left font-semibold text-[var(--text-2)]">Review Date</th>
                    </>
                  ) : (
                    <>
                      <th className="px-4 py-3 text-left font-semibold text-[var(--text-2)]">Order Code</th>
                      <th className="px-4 py-3 text-left font-semibold text-[var(--text-2)]">Name</th>
                      <th className="px-4 py-3 text-left font-semibold text-[var(--text-2)]">Unit</th>
                      <th className="px-4 py-3 text-left font-semibold text-[var(--text-2)]">Status</th>
                      <th className="px-4 py-3 text-left font-semibold text-[var(--text-2)]">Priority</th>
                      <th className="px-4 py-3 text-left font-semibold text-[var(--text-2)]">Progress</th>
                      <th className="px-4 py-3 text-left font-semibold text-[var(--text-2)]">Due Date</th>
                      <th className="px-4 py-3 text-left font-semibold text-[var(--text-2)]">Owner</th>
                    </>
                  )}
                </tr>
              </thead>
                  ...
                  <tr key={item.id || idx} className="hover:bg-slate-700/30 transition">
                    {selectedReport === "governance" ? (
                      <>
                        ...
                        <td className="px-4 py-3 text-[var(--text-2)]">{item.unit?.name || item.unit?.code || "—"}</td>
                        <td className="px-4 py-3 text-[var(--text-2)]">{item.type || "—"}</td>
                      ...
                      <>
                        ...
                        <td className="px-4 py-3 font-medium text-[var(--text)] max-w-[250px] truncate">{item.name}</td>
                        <td className="px-4 py-3 text-[var(--text-2)]">{item.unit?.name || item.unit?.code || "—"}</td>
                        <td className="px-4 py-3"><StatusBadge status={item.status} /></td>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs text-[var(--text-2)]">{item.percentComplete || 0}%</span>
                          </div>
                        </td>
                        <td className="px-4 py-3 text-[var(--text-2)] text-xs">
...
function StatusBadge({ status }: { status: string }) {
  const colors: Record<string, string> = {
    NOT_STARTED: "bg-slate-600/50 text-[var(--text-2)]",
    IN_PROGRESS: "bg-blue-500/20 text-blue-400",
    ON_HOLD: "bg-yellow-500/20 text-yellow-400",
    COMPLETED: "bg-green-500/20 text-green-400",
    CANCELLED: "bg-slate-600/50 text-[var(--text-3)]",
    DELAYED: "bg-red-500/20 text-red-400",
  };
  return (
    <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${colors[status] || "bg-slate-600/50 text-[var(--text-2)]"}`}>
      {status?.replace(/_/g, " ")}
    </span>
  );
}
...
function PriorityBadge({ priority }: { priority: string }) {
  const colors: Record<string, string> = {
    LOW: "bg-slate-600/50 text-[var(--text-2)]",
    MEDIUM: "bg-blue-500/20 text-blue-400",
    HIGH: "bg-orange-500/20 text-orange-400",
    CRITICAL: "bg-red-500/20 text-red-400 font-bold",
  };
  return (
    <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${colors[priority] || "bg-slate-600/50 text-[var(--text-2)]"}`}>
      {priority}
    </span>
  );
}
